...
def iterative_deepening_search(initial_node, gui=None, max_depth=20):
...
class PuzzleGUI:
        ...
        for i in range(3):
            for j in range(3):
                ...
                if val == 0:
                    ...
                    self.tiles[i][j].config(text=str(val), bg="#00f2ff") # Màu vàng nổi bật cho bản IDS

    def new_game(self):
        self.matrix = create_matrix() # Random ván mới
        self.update_ui()
        self.status_label.config(text="Sẵn sàng", fg="white")
    ...
    def animate_solution(self, steps, index=0):
        if index < len(steps):
            ...
            self.status_label.config(text=f"Bước: {steps[index].step} | Nước đi: {move_text} | Tổng: {len(steps)-1} bước", fg="white")
